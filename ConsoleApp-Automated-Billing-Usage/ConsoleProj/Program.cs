namespace ARMAPI_Test
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            */
            ...
            string requestURL = GetReportingUri(
                ...
                from: new DateTime(year: 2015, month: 08, day: 01, hour: 0, minute:0, second:0, kind: DateTimeKind.Utc),
                to: new DateTime(year: 2015, month: 12, day: 01, hour: 0, minute: 0, second: 0, kind: DateTimeKind.Utc),
        ...
        {
            var authenticationContext = new AuthenticationContext(string.Format("{0}/{1}", ConfigurationManager.AppSettings["ADALServiceURL"], ConfigurationManager.AppSettings["TenantDomain"]));

            X509Certificate2 cert = null;
            X509Store store = new X509Store(StoreName.My, StoreLocation.CurrentUser);
            string certName = ConfigurationManager.AppSettings["CertificateName"];
            try
            {
            ...
            var certCred = new ClientAssertionCertificate(ConfigurationManager.AppSettings["ClientID"], cert);
            var token = authenticationContext.AcquireToken("https://management.core.windows.net/", certCred);
            var creds = new TokenCloudCredentials(ConfigurationManager.AppSettings["SubscriptionID"], token.AccessToken);
